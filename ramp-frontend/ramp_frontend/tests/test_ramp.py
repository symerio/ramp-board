...
def test_sandbox_upload_file_dont_exist(
...
def test_sandbox_save_file(client_session, makedrop_event):
    ...
    with login_scope(client, "test_user", "test") as client:
        ...
        rv = client.post(
            "http://localhost/events/iris_test_4event/sandbox",
            headers={"Referer": "http://localhost/events/iris_test_4event/sandbox"},
            data={"estimator": example_code, "code-csrf_token": "temp_token"},
            follow_redirects=False,
        )
        assert rv.status_code == 200
...
def test_view_model(client_session):
    ...
    submission = get_submission_by_name(
        session, "iris_test", "test_user", "random_forest_10_10"
    )
...
def test_toggle_competition(client_session):
    ...
    event = session.query(Event).filter_by(name="iris_test").first()
    ...
    submission = (
        ...
        .filter_by(name="starting_kit_test", event_team_id=1)
        .first()
    )
