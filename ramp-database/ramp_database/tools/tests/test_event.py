...
@pytest.mark.parametrize("case, err_msg", [("unknown-extension", "Unknown extension")])
...
def _check_event(session, event, event_name, event_title, event_is_public, scores_name):
...
def test_delete_event(session_scope_function):
    ...
    event = get_event(session_scope_function, event_name)
    assert event

    # add event-team
    username = "test_user"
    sign_up_team(session_scope_function, event_name, username)
    event_team = get_event_team_by_name(session_scope_function, event_name, username)
    assert event_team
