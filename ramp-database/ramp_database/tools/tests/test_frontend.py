...
def test_is_accessible_event(session_toy_db, event_name, user_name, is_accessible):
    ...
    if event_name == "boston_housing_test":
        ...
        session_toy_db.commit()
    assert is_accessible_event(session_toy_db, event_name, user_name) is is_accessible
...
def test_user_signed_up(session_toy_db, event_name, user_name, is_accessible):
    assert is_user_signed_up(session_toy_db, event_name, user_name) is is_accessible
...
def test_is_user_sign_up_requested(session_toy_db, event_name, user_name, asked):
    assert is_user_sign_up_requested(session_toy_db, event_name, user_name) is asked
...
def test_is_accessible_code(session_toy_db):
    ...
    team = session_toy_db.query(Team).filter(Team.name == "test_user_3").first()
    user = session_toy_db.query(User).filter(User.name == "test_user_2").first()
    team.admin_id = user.id
    team.admin = user
    session_toy_db.commit()
    # check that the admin can access the submission
    assert is_accessible_code(session_toy_db, event_name, "test_user_2", sub.id)
    # but others cannot
    assert not is_accessible_code(session_toy_db, event_name, "test_user_3", sub.id)
...
def test_is_accessible_code_by_team_members(session_toy_db):
    ...
    sub = add_submission(
        session_toy_db, event_name, team_name, submission_name, path_submission
    )
    # check that the user submitting the submission could access it
    user = get_user_by_name(session_toy_db, "test_user_422")
    user.is_authenticated = True
    assert is_accessible_code(session_toy_db, event_name, "test_user_422", sub.id)
    # Check that another team member also can access the code
    user = get_user_by_name(session_toy_db, "test_user_423")
    user.is_authenticated = True
    assert is_accessible_code(session_toy_db, event_name, "test_user_423", sub.id)
...
def test_is_accessible_leaderboard(session_toy_db):
    ...
    user = get_user_by_name(session_toy_db, "test_iris_admin")
    user.is_authenticated = True
    assert is_accessible_leaderboard(session_toy_db, event_name, "test_iris_admin")
    # simulate a close event
    event = get_event(session_toy_db, event_name)
    event.closing_timestamp = datetime.datetime.utcnow()
    assert not is_accessible_leaderboard(session_toy_db, event_name, "test_user_2")
