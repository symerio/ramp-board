...
def export_leaderboards(config, event, path):
    ...
    with session_scope(config["sqlalchemy"]) as session:
        ...
        if not len(data):
            click.echo("No score was found on the submission {}".format(event))
        else:
            data.to_csv(path)
...
def delete_event(config, config_event, dry_run, from_disk, force):
    ...
    with session_scope(internal_config["sqlalchemy"]) as session:
        ...
        if db_event:
            ...
            click.echo("{} was removed from the database".format(event_name))
        if from_disk:
            if not db_event and not force:
                err_msg = (
                    "{} event not found in the database. If you want "
                    "to force removing event files from the disk, add "
                    'the option "--force".'.format(event_name)
                )
    ...
    with session_scope(config["sqlalchemy"]) as session:
        ...
        if submissions:
            ...
            for sub_info in submissions:
                sub = submission_module.get_submission_by_id(session, sub_info[0])
...
def compute_contributivity(config, config_event, min_improvement):
    ...
    with session_scope(config["sqlalchemy"]) as session:
        contributivity_module.compute_contributivity(
            session,
            event_name,
            config_event["ramp_kit_dir"],
            config_event["ramp_data_dir"],
            config_event["ramp_predictions_dir"],
            float(min_improvement),
        )
        contributivity_module.compute_historical_contributivity(session, event_name)
