...
def test_delete_event(make_toy_db, from_disk):
    ...
    assert (
        os.path.exists(event_path) if not from_disk else not os.path.exists(event_path)
    )


@pytest.mark.parametrize("force", [True, False])
@pytest.mark.parametrize("add_to_db", [True, False])
def test_delete_predictions(make_toy_db, database_connection, force, add_to_db):
    # check that delete event is removed from the database and optionally from
    # the disk
    runner = CliRunner()
    ramp_config = read_config(ramp_config_template())
...
def test_add_submission(make_toy_db):
    ...
    submission_path = os.path.join(
        ramp_config["ramp_kit_submissions_dir"], submission_name
    )
    shutil.copytree(
        os.path.join(ramp_config["ramp_kit_submissions_dir"], "random_forest_10_10"),
        submission_path,
    )
    runner = CliRunner()
    result = runner.invoke(
