...
def test_workflow_element_type_model(session_scope_module):
    workflow_element_type = (
        ...
        .filter(WorkflowElementType.type_id == SubmissionFileType.id)
    ...
    assert re.match(r"WorkflowElementType\(.*\)", repr(workflow_element_type))


@pytest.mark.parametrize("backref, expected_type", [("workflows", WorkflowElement)])
def test_workflow_element_type_model_backref(
...
def test_workflow_element_model(session_scope_module):
    workflow_element = (
        ...
        .filter(WorkflowElement.workflow_id == Workflow.id)
        .filter(Workflow.name == "Estimator")
        .one()
    )

    assert re.match(r"Workflow\(.*\): WorkflowElement\(.*\)", repr(workflow_element))
...
def test_workflow_element_model_backref(session_scope_module, backref, expected_type):
    workflow_element = (
        session_scope_module.query(WorkflowElement)
        .filter(WorkflowElement.workflow_id == Workflow.id)
        .filter(Workflow.name == "Estimator")
