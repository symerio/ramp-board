...
def test_submission_model_property(session_scope_module):
    ...
    assert all([isinstance(score, EventScoreType) for score in submission.score_types])
    ...
    assert os.path.join("submissions", submission_str) in submission.path
    ...
    assert submission.link == "/" + os.path.join(submission.hash_, "estimator.py")
    assert re.match(
        "<a href={}>{}/{}/{}</a>".format(
            submission.link,
    ...
    assert re.match(
        "<a href=.*{}.*error.txt>{}</a>".format(submission.hash_, submission.state),
...
def test_submission_model_set_state(session_scope_module):
    ...
    assert (submission.training_timestamp - datetime.utcnow()).total_seconds() < 10
...
def test_submission_model_set_state_after_training(
    session_scope_module, state_cv_folds, expected_state
):
    submission = get_submission_by_id(session_scope_module, ID_SUBMISSION)
...
def test_submission_score_model_property(session_scope_module):
    ...
    submission_score = (
        ...
        .filter(SubmissionScore.submission_id == ID_SUBMISSION)
        .first()
    )
...
def test_submission_score_model_backref(session_scope_module, backref, expected_type):
    submission_score = (
        session_scope_module.query(SubmissionScore)
        .filter(SubmissionScore.submission_id == ID_SUBMISSION)
        .first()
...
def test_submission_file_model_property(session_scope_module):
    ...
    submission_file = (
        ...
        .filter(SubmissionFile.submission_id == ID_SUBMISSION)
        .first()
    )
    assert re.match("SubmissionFile(.*)", repr(submission_file))
...
def test_submission_file_type_extension_model_property(session_scope_module):
    submission_file_type_extension = session_scope_module.query(
...
@pytest.mark.parametrize(
    "backref, expected_type", [("submission_files", SubmissionFile)]
)
def test_submission_file_type_extension_model_backref(
    session_scope_module, backref, expected_type
):
    submission_file_type_extension = session_scope_module.query(
        SubmissionFileTypeExtension
    ).first()
...
def test_submission_score_on_cv_fold_model_property(session_scope_module):
    cv_fold_score = (
        session_scope_module.query(SubmissionScoreOnCVFold)  # noqa
        .filter(
            SubmissionScoreOnCVFold.submission_score_id == SubmissionScore.id  # noqa
        )  # noqa
        .filter(
            SubmissionScore.event_score_type_id == EventScoreType.id  # noqa
        )  # noqa
...
def test_submission_on_cv_fold_model_property(session_scope_module):
    cv_fold = (
        ...
        .filter(SubmissionOnCVFold.submission_id == ID_SUBMISSION)
        .first()
    )
...
def test_submission_on_cv_fold_model_is_trained(
    session_scope_module, state_set, expected_state
):
    cv_fold = (
        session_scope_module.query(SubmissionOnCVFold)
...
def test_submission_on_cv_fold_model_is_validated(
    session_scope_module, state_set, expected_state
):
    cv_fold = (
        session_scope_module.query(SubmissionOnCVFold)
...
def test_submission_on_cv_fold_model_is_tested(
    session_scope_module, state_set, expected_state
):
    cv_fold = (
        session_scope_module.query(SubmissionOnCVFold)
...
def test_submission_on_cv_fold_model_is_error(
    session_scope_module, state_set, expected_state
):
    cv_fold = (
        session_scope_module.query(SubmissionOnCVFold)
...
@pytest.mark.parametrize(
    "backref, expected_type", [("scores", SubmissionScoreOnCVFold)]
)
def test_submission_on_cv_fold_model_backref(
    session_scope_module, backref, expected_type
):
    cv_fold = (
        session_scope_module.query(SubmissionOnCVFold)
...
@pytest.mark.parametrize(
    "backref, expected_type", [("workflow_element_types", WorkflowElementType)]
)
def test_submission_file_type_model_backref(
    session_scope_module, backref, expected_type
):
    submission_file_type = session_scope_module.query(SubmissionFileType).first()
